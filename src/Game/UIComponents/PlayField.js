...
class PlayField extends React.Component
{
    ...
    render()
    {
        ...
        return (
            <div className='col play-field mt-4 mt-lg-0'>
                <div className='row'>
                    <div className='col player-one'>
                        ...
                        <div>
                            ...
                            <div>{playerOne.id === this.props.socket.id && playerOne.cardsInPlay.length === 0 && <Button className='btn-sm mt-3' onClick={() => {this.fetchNextCard()}}>Flip Card</Button>}</div>
                        </div>
                    </div>
                    <div className='col-5 field'>
                        <div className='display-flex mt-5'>
                            ...
                            <div className='card-in-play w-50 d-inline-block text-right'>
                                {playerTwo.cardsInPlay && playerTwo.cardsInPlay.map((card, key) => <div className='play-card ml-auto' key={key} style={{color: card.suit.color}} dangerouslySetInnerHTML={{ __html: card.unicode}} />)}
                            </div>
                        </div>
                    </div>
                    <div className='col player-two text-right'>
                        ...
                        <div>
                            <div className='play-card ml-auto' style={{color: this.cardBack.color}} dangerouslySetInnerHTML={{ __html: this.cardBack.unicode}} />
                            <div>{playerTwo.id === this.props.socket.id && playerTwo.cardsInPlay.length === 0 && <Button className='btn-sm mt-3' onClick={() => {this.fetchNextCard()}}>Flip Card</Button>}</div>
                        </div>
                    </div>
                </div>
            </div>
